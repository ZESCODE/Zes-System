...
<div class="header">
  <div class="header-left">
    <button class="menu-btn" onclick="toggleDrawer()" style="background:var(--surface2);border:1px solid var(--border);color:var(--text2);width:34px;height:34px;border-radius:8px;cursor:pointer;font-size:16px;display:flex;align-items:center;justify-content:center">☰</button>
    <h1><span class="accent">ZES</span> Control Center <span style="color:var(--text3);font-weight:400;font-size:13px">v4</span></h1>
  </div>
...
<div id="mainContent">
  ...
  <div class="stats" id="statsBar">
    <div class="stat-card"><div class="num" id="statServices">0</div><div class="label">Services</div></div>
    <div class="stat-card"><div class="num" id="statRunning">0</div><div class="label">Running</div></div>
    <div class="stat-card"><div class="num" id="statProviders">0</div><div class="label">Providers</div></div>
    <div class="stat-card"><div class="num" id="statMCP">0</div><div class="label">MCP Servers</div></div>
  </div>

  <!-- Tabs -->
  <div class="tab-bar" id="tabBar">
    <button class="tab-btn active" data-tab="services">📊 Services</button>
    <button class="tab-btn" data-tab="providers">🔌 Providers <span class="badge" id="provBadge">18</span></button>
    <button class="tab-btn" data-tab="mcp">🧩 MCP <span class="badge" id="mcpBadge">6</span></button>
...
function renderProviders(providers) {
  ...
  grid.innerHTML = providers.map(p => {
    ...
    // Extract model info from providerSpecificData or modelLock keys
    let modelInfo = '';
    if (p.modelLock_) {
      modelInfo = Object.keys(p.modelLock_).join(', ').replace(/_/g, ' ');
    }
    // Check for modelLock_ keys in the object
    const modelKeys = Object.keys(p).filter(k => k.startsWith('modelLock_'));
    const models = modelKeys.map(k => k.replace('modelLock_', '').replace(/_/g, ' ')).join(', ');
    ...
    return `
      <div class="prov-card">
        ...
        <div class="info">
          ...
          ${models ? `<div style="font-size:9px;color:var(--blue);margin-top:1px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">🤖 ${models}</div>` : ''}
        </div>
      </div>
    `;
  }).join('');
